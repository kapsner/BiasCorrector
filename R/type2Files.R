...
type2_fileconfirm <- function(filelist, choiceslist, rv) {
  PCRBiasCorrection::writeLog_("Entered 'type2FileConfirm'-Function", logfilename = logfilename)

  rv$calibr_steps <- choiceslist[, ("step") := as.numeric(get("step"))][order(get("step"), decreasing = F)]

  if (rv$calibr_steps[, min(get("step"))] < 0 | rv$calibr_steps[, max(get("step"))] > 100) {
    ...
    gene_names <- unique(filelist[[rv$calibr_steps[1, get("name")]]][, c("locus_id", "CpG_count"), with = F])
    ...
    for (g in gene_names[, get("locus_id")]) {
      ...
      m <- data.table::data.table(matrix(nrow = 0, ncol = (as.numeric(gene_names[get("locus_id") == g, get("CpG_count")]) + 2)))
      # rename columns
    ...
    for (n in 1:nrow(rv$calibr_steps)) {
      ...
      for (locus in gene_names[, get("locus_id")]) {
        vec2 <- c(vec[2:(gene_names[get("locus_id") == locus, get("CpG_count")] + 1)], "row_means")
