moduleStatisticsServer <- function(input, output, session, rv, input_re){
  observe({
    ...
    output$description <- renderText({
      ...
      str3 <- "Column 2 presents the mean of the relative absolute errors for every CpG-site. It is calculated for every CpG-site as a mean of relative errors between the true and the observed methylation degrees of each methylation step across all available calibrator DNAs for the respective CpG-site. <br/>It is calculated by the formula<br/> <i>abs(methylation_true - methylation_observed) / methylation_true </i>"
